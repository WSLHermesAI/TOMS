...
int main() {
    ...
    // u8-prefixed: MSVC's plain (non-u8) narrow string literals interpret \uXXXX universal-
    // character-names through the SOURCE code page (1252 here), not UTF-8 -- every one of these 15
    // code points is unrepresentable in cp1252, so the compiler was silently substituting fallback
    // bytes instead of the intended 3-byte UTF-8 sequences (confirmed by the C4566 warning it
    // prints for every one of them), which silently broke the very CJK-wrap invariant this test
    // exists to prove (measured 2 lines where the correct wrap() output -- verified independently
    // in Python against the exact same algorithm -- gives 3). `u8"..."` has well-defined Unicode
    // semantics for \uXXXX regardless of the active code page.
    const std::string cjk = u8"\u5be9\u5224\u4e4b\u5370\u88ab\u6230\u9b25\u7684\u56de\u97ff\u9707\u788e\u4e86\u4e00\u89d2";   // 15 CJK glyphs
