...
void Game::inGameMenuClick(float x, float y) {
    ...
    if (inGameMenuPage_ == InGameMenuPage::Main) {
        // Each button's row index is found by position in mainMenuOrder() rather than a
        // hardcoded literal -- same reasoning as inGameMenuActivate()'s Main-page branch.
        auto order = mainMenuOrder();
        auto rowOf = [&](MainMenuRow r) -> int {
            auto it = std::find(order.begin(), order.end(), r);
            return (it != order.end()) ? (int)std::distance(order.begin(), it) : -1;
        };
        if (hit(igmSaveRect_)) { inGameMenuSel_ = rowOf(MainMenuRow::Save); inGameMenuActivate(); return; }
        if (hit(igmSettingsRect_)) { inGameMenuSel_ = rowOf(MainMenuRow::Settings); inGameMenuActivate(); return; }
        if (hit(igmSkillsRect_)) { inGameMenuSel_ = rowOf(MainMenuRow::Skills); inGameMenuActivate(); return; }
        if (hubMenuUnlocked() && hit(igmHubRect_)) { inGameMenuSel_ = rowOf(MainMenuRow::Village); inGameMenuActivate(); return; }
        if (forgeMenuUnlocked() && hit(igmForgeRect_)) { inGameMenuSel_ = rowOf(MainMenuRow::Forge); inGameMenuActivate(); return; }
        if (hit(igmBackToTitleRect_)) { inGameMenuSel_ = rowOf(MainMenuRow::BackToTitle); inGameMenuActivate(); return; }
        if (hit(igmCloseRect_)) { closeInGameMenu(); return; }
    } else if (inGameMenuPage_ == InGameMenuPage::Skills) {
        auto order = skillMenuOrder();
        int n = std::min((int)igmSkillRowRects_.size()/4, (int)order.size());
        for (int i = 0; i < n; i++) {
            const float* r4 = &igmSkillRowRects_[i*4];
        ...
        auto order = forgeMenuOrder();
        int n = std::min((int)igmForgeRowRects_.size()/4, (int)order.size());
        for (int i = 0; i < n; i++) {
            const float* r4 = &igmForgeRowRects_[i*4];
            int r[4] = {(int)r4[0], (int)r4[1], (int)r4[2], (int)r4[3]};
            if (hit(r)) { inGameMenuSel_ = i; inGameMenuActivate(); return; }
        }
        if (hit(igmBackRect_)) { inGameMenuBack(); return; }
        ...
        auto order = hubMenuOrder();
        int n = std::min((int)igmHubRowRects_.size()/4, (int)order.size());
        for (int i = 0; i < n; i++) {
            const float* r4 = &igmHubRowRects_[i*4];
            int r[4] = {(int)r4[0], (int)r4[1], (int)r4[2], (int)r4[3]};
            if (hit(r)) { inGameMenuSel_ = i; inGameMenuActivate(); return; }
        }
        if (hit(igmBackRect_)) { inGameMenuBack(); return; }
...
void Game::drawInGameMenu() {
    ...
    if (inGameMenuPage_ == InGameMenuPage::Main) {
        // S5/S6: row count/labels/rects driven entirely by mainMenuOrder() -- adding a third
        // conditional row later (a future hub location gets its own Main-page entry, say) means
        // adding one MainMenuRow case here and to mainMenuOrder(), not another bh/index branch.
        auto order = mainMenuOrder();
        const float rw = 420 - 60, rh = 64, gap = 16;
        const float bw = 420, bh = 90.0f + order.size() * (rh + gap) + 20.0f + 48.0f + 20.0f;
        ...
        const float rx = bx + 30;
        float ry = by + 90;
        for (size_t i = 0; i < order.size(); i++) {
            bool sel = (int)i == inGameMenuSel_;
            switch (order[i]) {
                case MainMenuRow::Save:
                    button(igmSaveRect_, rx, ry, rw, rh, locale_.tr("ingame_menu.save"), sel); break;
                case MainMenuRow::Settings:
                    button(igmSettingsRect_, rx, ry, rw, rh, locale_.tr("menu.settings"), sel); break;
                case MainMenuRow::Skills:
                    button(igmSkillsRect_, rx, ry, rw, rh, locale_.tr("ingame_menu.skills"), sel); break;
                case MainMenuRow::Village:
                    button(igmHubRect_, rx, ry, rw, rh, locale_.tr("ingame_menu.village"), sel); break;
                case MainMenuRow::Forge:
                    button(igmForgeRect_, rx, ry, rw, rh, locale_.tr("ingame_menu.forge"), sel); break;
                case MainMenuRow::BackToTitle:
                    button(igmBackToTitleRect_, rx, ry, rw, rh, locale_.tr("ingame_menu.back_to_title"), sel); break;
            }
            ry += rh + gap;
        }
        ...
        // S6: mirrors the Forge page layout/pattern above almost exactly, but rows are hub
        // locations (no cost -- either the run flag is set, or it isn't) instead of recipes.
        auto order = hubMenuOrder();
        const float rh = 56, gap = 10;
        auto L = computeSubPageLayout(W, H, (int)order.size(), rh, gap);
        const float bw = L.panel.w, bh = L.panel.h, bx = L.panel.x, by = L.panel.y;
        panel(bx, by, bw, bh);
        const std::string head = locale_.tr("ingame_menu.village");
        drawText(head, bx + (bw - measureText(head, 26)) * 0.5f, by + 26.0f, 26, gold);
        const float rw = L.rowW, rx = L.rowX;
        float ry = L.rowY0;
        igmHubRowRects_.assign((size_t)order.size() * 4, 0.0f);
        ...
        for (size_t i = 0; i < order.size(); i++) {
            const auto& def = hubDefs_.at(order[i]);
            bool unlocked = toms::hubLocationUnlocked(hubDefs_, hasFlag, order[i]);
            std::string mark = unlocked ? "[ ] " : "[--] ";
            std::string name = unlocked ? locale_.field(def.name) : locale_.tr("hub.unknown_location");
            button(scratch, rx, ry, rw, rh, mark + name, (int)i == inGameMenuSel_);
            if (!unlocked) {
                Quad dim2; dim2.rect[0]=rx; dim2.rect[1]=ry; dim2.rect[2]=rw; dim2.rect[3]=rh;
                dim2.uv[0]=0; dim2.uv[1]=0; dim2.uv[2]=1; dim2.uv[3]=1; dim2.solid=true;
                dim2.tint[0]=0.05f; dim2.tint[1]=0.05f; dim2.tint[2]=0.07f; dim2.tint[3]=0.55f;
                ren->drawSprite(dim2);
            }
            igmHubRowRects_[i*4+0] = rx; igmHubRowRects_[i*4+1] = ry;
            igmHubRowRects_[i*4+2] = rw; igmHubRowRects_[i*4+3] = rh;
            ry += rh + gap;
        }
        const float cw=140, ch=44, cx=bx+(bw-cw)*0.5f;
        button(igmBackRect_, cx, ry + 10, cw, ch, locale_.tr("menu.back"), false);
    } else if (inGameMenuPage_ == InGameMenuPage::Forge) {
        // S5: mirrors the Skills page layout/pattern above almost exactly, but rows are recipes
        // (gold + material costs) instead of skill-tree nodes.
        auto order = forgeMenuOrder();
        const float rh = 56, gap = 10;
        auto L = computeSubPageLayout(W, H, (int)order.size(), rh, gap);
        const float bw = L.panel.w, bh = L.panel.h, bx = L.panel.x, by = L.panel.y;
        panel(bx, by, bw, bh);
        const std::string head = locale_.tr("ingame_menu.forge");
        drawText(head, bx + (bw - measureText(head, 26)) * 0.5f, by + 26.0f, 26, gold);
        std::string goldLabel = trParam(locale_.tr("forge.gold_label"), "n", std::to_string(pl.gold));
        drawText(goldLabel, bx + (bw - measureText(goldLabel, 16)) * 0.5f, by + 62.0f, 16, dim);
        const float rw = L.rowW, rx = L.rowX;
        float ry = L.rowY0;
        igmForgeRowRects_.assign((size_t)order.size() * 4, 0.0f);
        int scratch[4];   // button() wants an int[4] out-param; the real rect is stored below
        for (size_t i = 0; i < order.size(); i++) {
            const auto& def = forgeDefs_.at(order[i]);
            bool known = std::find(meta_.forgeRecipesKnown.begin(), meta_.forgeRecipesKnown.end(), order[i])
                != meta_.forgeRecipesKnown.end();
            bool craftable = known && toms::canCraft(forgeDefs_, meta_.forgeRecipesKnown, pl.inv, pl.gold, order[i]);
            std::string mark = !known ? "[--] " : (craftable ? "[ ] " : "[!] ");
            std::string name = known ? locale_.field(def.name) : locale_.tr("forge.unknown_recipe");
            std::string label = mark + name;
            if (known) label += " (" + std::to_string(def.costGold) + "g)";
            button(scratch, rx, ry, rw, rh, label, (int)i == inGameMenuSel_);
            if (!craftable) {
                Quad dim2; dim2.rect[0]=rx; dim2.rect[1]=ry; dim2.rect[2]=rw; dim2.rect[3]=rh;
                dim2.uv[0]=0; dim2.uv[1]=0; dim2.uv[2]=1; dim2.uv[3]=1; dim2.solid=true;
                dim2.tint[0]=0.05f; dim2.tint[1]=0.05f; dim2.tint[2]=0.07f; dim2.tint[3]=0.55f;
            ...
            igmForgeRowRects_[i*4+0] = rx; igmForgeRowRects_[i*4+1] = ry;
            igmForgeRowRects_[i*4+2] = rw; igmForgeRowRects_[i*4+3] = rh;
            ry += rh + gap;
        }
        const float cw=140, ch=44, cx=bx+(bw-cw)*0.5f;
        button(igmBackRect_, cx, ry + 10, cw, ch, locale_.tr("menu.back"), false);
    } else if (inGameMenuPage_ == InGameMenuPage::Skills) {
        // S4: a flat list (skillMenuOrder()'s order -- grouped by lineage in story order, root
        // before its own deeper tiers), each row showing owned/unlockable/locked state. Reuses the
        // Settings page's row-list layout/panel-sizing pattern below almost exactly.
        auto order = skillMenuOrder();
        const float rh = 56, gap = 10;
        auto L = computeSubPageLayout(W, H, (int)order.size(), rh, gap);
        const float bw = L.panel.w, bh = L.panel.h, bx = L.panel.x, by = L.panel.y;
        panel(bx, by, bw, bh);
        const std::string head = locale_.tr("ingame_menu.skills");
        drawText(head, bx + (bw - measureText(head, 26)) * 0.5f, by + 26.0f, 26, gold);
        std::string ptsLabel = trParam(locale_.tr("skill.points_label"), "n", std::to_string(run_.skillPoints()));
        drawText(ptsLabel, bx + (bw - measureText(ptsLabel, 16)) * 0.5f, by + 62.0f, 16, dim);
        const float rw = L.rowW, rx = L.rowX;
        float ry = L.rowY0;
        igmSkillRowRects_.assign((size_t)order.size() * 4, 0.0f);
        int scratch[4];   // button() wants an int[4] out-param; the real rect is stored below
        for (size_t i = 0; i < order.size(); i++) {
            const auto& def = skillDefs_.at(order[i]);
            bool owned = run_.hasSkill(order[i]);
            bool unlockable = !owned && toms::canUnlockSkill(skillDefs_, run_.skillsOwned(), run_.skillPoints(), order[i]);
            std::string mark = owned ? "[x] " : (unlockable ? "[ ] " : "[--] ");
            std::string name = locale_.field(def.name);
            std::string label = owned ? (mark + name)
                : trParam(mark + name + " (" + locale_.tr("skill.cost_suffix") + ")", "n", std::to_string(def.cost));
            button(scratch, rx, ry, rw, rh, label, (int)i == inGameMenuSel_);
            // A locked row (missing prerequisite, or already owned) is drawn but visually muted --
            // matches the store's own "can't afford it yet" convention rather than hiding the row,
            // so the tree's shape is visible before every node is actually reachable.
            if (!owned && !unlockable) {
                Quad dim2; dim2.rect[0]=rx; dim2.rect[1]=ry; dim2.rect[2]=rw; dim2.rect[3]=rh;
                dim2.uv[0]=0; dim2.uv[1]=0; dim2.uv[2]=1; dim2.uv[3]=1; dim2.solid=true;
                dim2.tint[0]=0.05f; dim2.tint[1]=0.05f; dim2.tint[2]=0.07f; dim2.tint[3]=0.55f;
            ...
            igmSkillRowRects_[i*4+0] = rx; igmSkillRowRects_[i*4+1] = ry;
            igmSkillRowRects_[i*4+2] = rw; igmSkillRowRects_[i*4+3] = rh;
            ry += rh + gap;
        }
        const float cw=140, ch=44, cx=bx+(bw-cw)*0.5f;
        button(igmBackRect_, cx, ry + 10, cw, ch, locale_.tr("menu.back"), false);
    } else if (inGameMenuPage_ == InGameMenuPage::Settings) {
        const auto& langs = locale_.languages();
        ...
        auto L = computeSubPageLayout(W, H, n + 1, rh, gap, /*panelW=*/420.0f);
        const float bw = L.panel.w, bh = L.panel.h, bx = L.panel.x, by = L.panel.y;
        panel(bx, by, bw, bh);
