...
> 搭配 `docs/story/STORY_BIBLE.md`（70 層／10 幕／15 結局／輪迴）與 `docs/story/SIDE_STORIES.md`（10 條旗艦支線＋事件池）。
...
### 5.1 樓層表（`tools/gen_floors.py` 的輸入，與 `docs/story/STORY_BIBLE.md` §5 一致）
...
> 表格（`docs/story/STORY_BIBLE.md` §5 與驗證器 V9 的依據）才是權威，因此公式以下表的**分段精確式**取代，
> 已改為與表格一致：
...
### 5.1b 每層尺寸都不相同（2026-09-13 業主指示）

業主明確要求：**每一層的格數都要不同、越高層越大**（例：第 1 層 10×10、第 2 層 10×12、第 3 層 14×14），
**只有頭目層不套用**——頭目層本來就沒有生成圖，它們載入手寫的 `data/stages/stageNN.json`，天然豁免。

實作：`tools/gen_floors.py` 的 `SIZE_RAMP` 取代原本「同一幕七層共用一個尺寸」的規則，把 44 個欄步 + 26 個
列步（共 70 個增量）平攤到 F01→F70 的 69 段，因此**每層至少長一格、且 70 層尺寸互不相同**：
F01 19×16 → F02 20×16 → F03 20×17 … → F70 63×42（塔的總跨度不變）。產生器對不在 3c+1×3r+1 格點上的
目標尺寸會在遠端補牆（最多約 2 格），所以任意單調尺寸都合法。`FLOOR_TABLE` 的 rooms/loops/events/
敵人混合/道具仍舊按幕成長。
...
### 5.3 樓梯對齊（2026-09-16 業主指示）

業主明確要求：**上下樓梯的位置要對得起來**——第 N 層的「上樓梯」瓦片座標，必須跟第 N+1 層的「下樓梯」
瓦片座標完全相同（反之亦然），而且**玩家走樓梯抵達新樓層時，要真的出現在那個座標上**，不是回到該層
寫死的預設出生點。等於把上下樓層當成同一根樓梯井的兩端，物理位置對齊，而非各自獨立亂數。

實作分兩塊，細節見 `docs/story/STAIR_ALIGNMENT.md`（英文，寫給維護產生器/引擎的工程師）：

1. **資料面**：`tools/gen_stairs.py`（新增，取代逐層各自呼叫 `gen_mazes.py`/`gen_floors.py` 迷宮演算法
   的舊流程）依 F01→F70 嚴格順序重新產生 70 層的迷宮**形狀**——樓層尺寸（§5.1b 的 SIZE_RAMP）、每層的
   怪物／道具／NPC／事件／鑰匙與門**維持不變，只重新排版**。第 N 層算出的「上樓梯」格子，直接當作第
   N+1 層的強制「入口」格子（迷宮格子座標只跟 (row, col, scale) 有關、跟整張圖多大無關，所以較小樓層
   算出的座標，套到較大樓層一樣合法）。頭目層（`data/stages/stageNN.json`）也在同一條鏈上，
   `stage_11.json` 例外——它是打贏頭目後才會用 `Game::finishCombatWin()` 直接跳轉的結局畫面，不是走
   樓梯到得了的地方，不參與這條鏈。
2. **引擎面**：`Game::loadStage()` 新增 `StageArrival` 參數（`Fresh`／`FromBelow`／`FromAbove`）。
   `confirmStageTransition()` 從「往上走」改叫 `FromBelow`（新樓層的下樓梯，樓梯井從下面接上來）、
   「往下走」改叫 `FromAbove`（新樓層的上樓梯）；除此之外的每個 `loadStage()` 呼叫（新遊戲、輪迴、
   Stage Select、除錯用的 `jsGoStage`）維持 `Fresh`，行為不變——不管有沒有 `'@'`，都有真正能站的格子
   可以退回去用。

第 1 層是整條鏈的起點，沒有下樓梯（`connect.down` 本來就是 `null`），現在也真的沒有 `'D'` 瓦片了（過去
產生器仍會放一個，但因為 `FloorTable::prev("F01")` 永遠回傳空字串，那個瓦片一直是死的）。
...
| `docs/design/MAIN_BATTLE_SCENE_DESIGN.md` / `docs/design/BATTLE_SYSTEM_V2_PROPOSALS.md` | 戰鬥機制；`bar`、`actives` 建立其上 |
| `docs/architecture/NODE_SYSTEM.md` | `beats[].kind` 沿用其語意 |
| `story_controller.h` / `mission_system.h` / `condition.h` / `save_system.h` | 需擴充的四個引擎模組（§2.2、§9、§12.5） |
| `docs/story/STORY_BIBLE.md` / `docs/story/SIDE_STORIES.md` | 敘事與支線來源（本檔為其資料契約） |
