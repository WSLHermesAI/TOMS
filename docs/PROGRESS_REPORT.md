...
**M8 (Content authoring) is done.** The 9-item equipment set is sellable in the Store and actually
equippable, the 3-mission set is offered/claimable through NPC dialogue, 3 monster types now talk
before fighting via `dialogue_gate`, and all 11 floors show recommended-stats text in Stage Select.
Getting there required two small, owner-approved system additions the roadmap hadn't anticipated
(a `claimMission` dialogue verb, and selling equipment through the Store to make it acquirable at
all) plus fixes for 3 real gaps found along the way (mission rewards were a dead end, equipment was
never loadable or equippable, and `applyEquipmentStats()` was built but never called). See the
latest log entry for full detail, including two unrelated pre-existing dead-content bugs found
(and one fixed) while authoring this.

**This is the milestone that most needs the owner's hands-on time before going further** — almost
none of it can be verified from here beyond "it compiles, loads without JSON errors, and the
regression still passes":
- Open the shop and confirm the new tab bar (藥水/武器/防具/天賦) looks right and buying an item
  actually equips it (watch for the "✓ 已裝備" tag).
- Talk to the old sorcerer (floor 2) and the village elder (floor 1, and again once you've reached
  floor 5) and try accepting/completing/claiming a mission.
- Pick a fight with a skeleton (floors 4/5/7), a wraith (floors 5-9, try the "listen" option too),
  and a demon (floors 6-9) to confirm the new dialogue actually shows before combat starts.

**Also still open from before M8:** the Power Bar combat feel in general (M6), the M7 entity-status
fix (defeat a monster or grab an item, leave the floor, come back, confirm it stays gone), the
F12/Steam-overlay test, and Milestone 5's deferred Inventory/Shop/Dialogue ImGui migration.

**M9 (Platform verification & polish) is now in progress.** Its first checklist item — full
regression on both Windows and web build targets — is done for the first time ever (see the latest
log entry: a working Emscripten SDK was found/upgraded this session, and both WebGL and WebGPU now
build and pass their headless tests). Mid-milestone, the owner also asked for every stage's maze to
be regenerated by Wilson's algorithm with size growing by floor — also done, see the same log entry.

**What still needs the owner's hands-on time, in rough priority order:**
- **The new mazes specifically** — walk all 11 floors (or at least a sample: floor 1 for the
  "should look identical to before" claim, floor 6 for the door/key puzzle, floor 10 for the boss
  placement, floor 11 for the two-NPC epilogue) and confirm they're solvable, monsters/items/NPCs
  are reachable, and bigger floors' shrunk tile size still looks/reads fine.
- Everything still outstanding from M8 (shop tabs/equip, mission accept/claim, the 3 new
  `dialogue_gate` fights) and M7 (the entity-status "stays cleared" fix) — none of this changed
  this session, still needs a look.
- The web build, in an actual browser (`web-gl/toms_web.html` or `web-gpu/toms_web.html`, served
  ...
  WASM, but nothing about actual WebGL/WebGPU rendering has been checked yet.
