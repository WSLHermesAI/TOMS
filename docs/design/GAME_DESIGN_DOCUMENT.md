...
> **Scope of this document.** `README.md`, `docs/design/FIGHTING_TALKING_DESIGN.md`, `docs/architecture/NODE_SYSTEM.md`, and
...
`docs/design/FIGHTING_TALKING_DESIGN.md`).
...
## 7. Combat System *(Established — `data/combat.json`, `docs/design/FIGHTING_TALKING_DESIGN.md` §1)*

> **Update:** the passive "auto-punch every 700ms" round described below is proposed to become an
> active, skill-timed exchange — see [`docs/design/FIGHT_SCENE_DESIGN.md`](docs/design/FIGHT_SCENE_DESIGN.md) (the
> Attack/Defense Power Bar minigame) and [`docs/design/MAIN_BATTLE_SCENE_DESIGN.md`](docs/design/MAIN_BATTLE_SCENE_DESIGN.md)
...
triggers an automatic level-up (`atk += 2`, `def += 1` per `docs/design/FIGHTING_TALKING_DESIGN.md` §1.4).
...
`docs/architecture/IMPLEMENTATION_ROADMAP.md` Milestone 7)*
...
`docs/design/FIGHT_SCENE_DESIGN.md` §4 documents as reproducing this exact pre-Power-Bar formula, making this
...
- **Shop economy vs. the new Equipment System:** `docs/design/GAME_DESIGN_DOCUMENT.md` §9's "gold is a
...
## 8. Talking / Dialogue System *(Established — `docs/design/FIGHTING_TALKING_DESIGN.md` §2)*
...
1. **Never place an unwinnable-by-the-numbers fight as mandatory.** Every monster blocking the
   ...
   `docs/design/FIGHTING_TALKING_DESIGN.md` §4's open checklist item).
...
- **Inventory (9-grid, per `docs/architecture/NODE_SYSTEM.md`)** should show each held item's effect text
...
1. **Explicit level-up thresholds** (§7) — close the one genuinely undefined number in the core
   loop.
2. **Balance verification pass** against `docs/design/FIGHTING_TALKING_DESIGN.md` §4's checklist — hand-trace
...
Carried forward from `docs/design/FIGHTING_TALKING_DESIGN.md` §4, closed against real data in Milestone 7
(`docs/architecture/IMPLEMENTATION_ROADMAP.md`) — see §7's "Balance verification pass" above for the actual
...
- [x] Combat outcomes are 100% determined by atk/def/hp — no hidden randomness anywhere. ✅ Still
      ...
      not a seeded roll (`docs/design/FIGHT_SCENE_DESIGN.md`'s own stated invariant).
...
- `TOMS/README.md`, `TOMS/FIGHTING_TALKING_DESIGN.md`, `TOMS/docs/architecture/NODE_SYSTEM.md`, `TOMS/docs/WORKING_LOG.md`
