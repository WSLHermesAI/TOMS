...
> combat animation described in `docs/design/FIGHTING_TALKING_DESIGN.md` §1 with an **active, skill-timed**
...
> `docs/design/GAME_DESIGN_DOCUMENT.md` Pillar 1) — it adds a **player-controlled variance** on top of the
...
The current fight scene (`docs/design/FIGHTING_TALKING_DESIGN.md` §1.3) is a fixed 700ms-per-round animation
...
`docs/design/MAIN_BATTLE_SCENE_DESIGN.md`) — a weapon doesn't just change `V0`/`Vmax`/`rampTime` (how fast the
...
(`docs/design/FIGHTING_TALKING_DESIGN.md` §1.3) exactly — it just turns each half of the round into a timed
...
`docs/design/MAIN_BATTLE_SCENE_DESIGN.md` §4 (Equipment System): `V0`/`Vmax`/`rampTime` change the shape of
...
- [ ] Should `greenHalf`/`blueOuter`/`redOuter` ever shrink to near-0 for a specific enemy (e.g. the boss) so
      ...
      `docs/design/MAIN_BATTLE_SCENE_DESIGN.md` §4 — worth revisiting once the boss fight is played by hand.)
