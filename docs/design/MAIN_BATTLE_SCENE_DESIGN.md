...
> new Attack/Defense Power Bar (`docs/design/FIGHT_SCENE_DESIGN.md`) and a new **Equipment System** fit into a
> single round of combat. This extends `docs/design/FIGHTING_TALKING_DESIGN.md` §1 and §3 and the combat
> section of `docs/design/GAME_DESIGN_DOCUMENT.md` §7 — it does not replace their underlying stat model.
>
...
> computed with the exact formulas in this document and `docs/design/FIGHT_SCENE_DESIGN.md`, not scripted.
...
Overworld (13×11 floor grid)
   └─ player walks onto a monster tile
        └─ BATTLE SCENE  ◄──────────────── this document
             ...
             ├─ round loop:
             │    ├─ Attack Bar  → damage to enemy   (docs/design/FIGHT_SCENE_DESIGN.md §4)
             │    ├─ (enemy dead? → WIN, break)
             │    ├─ Defense Bar → damage to player   (docs/design/FIGHT_SCENE_DESIGN.md §4)
...
Per `docs/design/GAME_DESIGN_DOCUMENT.md` §13 (UI/HUD Design), the HUD (stage name, ATK/DEF/LV/EXP/keys/HP)
...
- **Bottom-center:** the **active Power Bar** for whichever side is currently acting (Attack Bar
  ...
  with the interactive bar described in `docs/design/FIGHT_SCENE_DESIGN.md`.
- **Bottom:** narrative/flavor line area (unchanged — this is where enemy dialogue lines like
  golem's "此地不容活物" already render per `docs/design/FIGHTING_TALKING_DESIGN.md`).
...
2. **Attack Bar appears**, already zoned (§2 of `docs/design/FIGHT_SCENE_DESIGN.md`); player holds and
   releases.
...
Per `docs/design/FIGHT_SCENE_DESIGN.md` §2, every Power Bar is symmetric around its center: green in the
...
`docs/architecture/NODE_SYSTEM.md`'s 9-grid UI). This mirrors `docs/design/GAME_DESIGN_DOCUMENT.md` §13's requirement that item
...
exactly the kind of decision this project's skill-first pillar (`docs/design/GAME_DESIGN_DOCUMENT.md` Pillar 1
extension, see `docs/design/FIGHT_SCENE_DESIGN.md` intro) should reward.
...
mandatory fight must be winnable" checklist item already in `docs/design/GAME_DESIGN_DOCUMENT.md` §17.
...
`docs/design/FIGHT_SCENE_DESIGN.md` §5 — `rampTime` is the cubic ease-in's "seconds to reach Vmax," i.e. how
...
Extends `docs/design/GAME_DESIGN_DOCUMENT.md` §17. Status as of Milestone 7's balance pass
...
- [x] Every bestiary entry hand-traced at P≈44% (the "old-parity" release point) still matches
      ...
      `docs/design/GAME_DESIGN_DOCUMENT.md` §7's "Balance verification pass" for the numbers. Comfortable
