...
## 4. 數值平衡檢查清單（設計／測試）— 已於 Milestone 7 用實際 `data/*.json` 模擬驗證，
細節見 `docs/design/GAME_DESIGN_DOCUMENT.md` §7、§17
- [x] 任意路線都存在「可由公式驗證的可通關解」（先吃寶石再打怪）。已驗證：完整撿取寶石時全程
      有充足容錯；完全不撿寶石則第 6 層起（以及 Boss）會直接死亡——這正是設計本身要求的取捨，
      不是漏洞。真正的「哪些寶石是硬性必需」仍需地圖連通性分析才能精確定位，暫列為殘留項目。
- [x] 第 10 層 Boss（HP 400 / atk 34 / def 14）在玩家典型成長下可擊敗。已驗證：完整撿取路線
      下，玩家到達 Boss 時 ATK 58 / DEF 34，此戰僅承受約 9 點傷害，容錯極大。
- [x] 戰鬥結果完全由 atk/def/hp 決定，無隱藏隨機。Milestone 6 加入 Power Bar 後仍成立——新增
      的變因是玩家操作的放開時機，不是種子隨機。
- [x] 對話 `next` 鏈無懸空（指向不存在的節點）。已對 `data/dialogue/*.json` 全部檔案程式化
      檢查，未發現懸空節點；也確認目前沒有任何對話檔案使用 Milestone 4 新增的 `action` 欄位。
