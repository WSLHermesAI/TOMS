...
| **Rewrite on adopted libraries** | the two mains → SDL3 app (L1); three renderers + `render_iface.h` → Diligent (L3); immediate-mode UI + ~30 hit-rect fields → RmlUi/widgets (L5); the `Game` god object → scenes + services ([03 §2](03_GAME_LAYER.md#2-what-replaces-the-game-god-object)); the web page JS in strings → `shell.html` + `engine.js` |
| **Retire** | the Qt `editor/` (after 9.3); `texture.*` (unused); `make_item_icons.py`, `make_missing_sprites.py`, root `gen_content.py` / `gen_textures.py` / `make_review_pdf.py`; old build output folders (`web-gl/`, `web-gpu/`, `web/` versioned bundles) |
...
| 2 | WebGPU `flush()` clears and presents on every call, so the text pass wipes the sprites | `src/engine/renderer_webgpu.cpp` | clear once per frame, present in `end()` |
| 3 | WebGPU `init` gets 1280×720 instead of the 1024×768 design size; there is no `updateFont` | `renderer_webgpu.cpp` | pass the design size; implement `updateFont` |
...
|---|---|---|---|
| **E-M0 Skeleton** | 0.1–0.6, 1.1–1.3, 1.5, 2.1, 2.3, 2.4, 3.1 (clear + quad), Q.1, the VS workflow ([05](05_DEV_WORKFLOW_VS.md)) | F5 desktop (Vulkan) and F5 web (WebGPU **and** WebGL2) show a textured quad; Test Explorer lists all tests; CI builds both; natvis works; the save overwrite test passes. **Includes the RHI risk check** ([09 §2](09_RENDERING_2D_3D.md#2-decision-card-31-rhi-graphics-api-abstraction)): Diligent web size/stability, or fall back to sokol | 2–3 weeks |
...
| **E-M4 Editor + prefabs** | O.1–O.3, 9.1–9.4 | in-game editor on desktop and web: hierarchy, inspector, prefabs with overrides, UI placement, **C++ method binding + Event System binding**, play-in-editor; LDtk levels import; the event editor + simulator work | 4–6 weeks |
