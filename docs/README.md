...
| [`EngineBlueprint/`](EngineBlueprint/) | **Planning doc for the next project**: a reusable C++ engine with Magic Tower as its first game. Covers the layered system hierarchy (72 engine/tool systems + 12 game systems), build-vs-adopt cards, third-party library shortlist, resource/lifetime management, 2D+3D rendering on **bgfx**, the **Qt6 desktop editor** (embedded bgfx viewport, not shipped) with prefabs and code binding, UI-authoring tools and 3D model support for bgfx, the Visual Studio workflow, and a milestone plan. Start at `EngineBlueprint/README.md`. |
| [`EventSystem/`](EventSystem/) | **Proposed** data-driven Event System (Trigger → Steps → Fire): the flow JSON schema, how the runtime hooks into `Game`, the Qt event editor, and how it previews/simulates a whole event flow. Start at `EventSystem/README.md`. Not built yet. |
