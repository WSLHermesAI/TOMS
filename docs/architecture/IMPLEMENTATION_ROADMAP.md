...
> `docs/architecture/GAME_LOGIC_AND_RENDERING_ARCHITECTURE.md` (system list, state machine, UI framework decision)
> plus the still-open items in `docs/design/GAME_DESIGN_DOCUMENT.md` §16–17, `docs/design/FIGHT_SCENE_DESIGN.md` §7, and
> `docs/design/MAIN_BATTLE_SCENE_DESIGN.md` §7 into buildable, testable milestones. Each milestone lists what it
...
> WebGPU ImGui window could be written but never seen. Tracked in docs/progress_report/PROGRESS_REPORT.md's M2 row.
...
> game's own UI code — see the M5 row in docs/progress_report/PROGRESS_REPORT.md.
...
This is the highest-risk milestone — it's the one item flagged in `docs/design/MAIN_BATTLE_SCENE_DESIGN.md` §6
...
| Power Bar runtime | Implement the cubic ease-in speed curve + 3-zone geometry from `docs/design/FIGHT_SCENE_DESIGN.md` §2, driven by real input instead of synthetic release percentages |
| `data/equipment.json` | Weapon/Armor/Talent schema per `docs/design/MAIN_BATTLE_SCENE_DESIGN.md` §4.4; wire into §6.2's effective-stat stack |
| Battle Scene screen assembly | Layout per `docs/design/MAIN_BATTLE_SCENE_DESIGN.md` §2–3: persistent HUD + actor sprites (scene-graph, unchanged) + active Power Bar (scene-graph, custom-drawn per architecture-doc §2.3 Phase 5 — **not** ImGui, for frame-timing safety) + HUD numbers/flee button (ImGui) |
...
- [ ] Explicit EXP level-up thresholds, replacing `docs/design/GAME_DESIGN_DOCUMENT.md` §7's proposed-but-
...
- [ ] Hand-trace every mandatory fight winnable with stats obtainable on that floor or before
      (`docs/design/GAME_DESIGN_DOCUMENT.md` §11.1 / §17).
- [ ] Confirm floor-10 boss (HP 400/ATK 34/DEF 14) beatable under typical growth, with and without
      equipment (`docs/design/MAIN_BATTLE_SCENE_DESIGN.md` §7).
- [ ] Confirm every bestiary entry hand-traced at P≈44% Power Bar release still matches
      pre-Power-Bar balance (`docs/design/FIGHT_SCENE_DESIGN.md` §7 / `docs/design/MAIN_BATTLE_SCENE_DESIGN.md` §7).
...
- [ ] Confirm shop cost curve still keeps gold a top-up, not a primary growth path
      (`docs/design/GAME_DESIGN_DOCUMENT.md` §9), given equipment now adds a second gold sink/reward path.
...
- [ ] Author `data/equipment.json`'s starting set — the 9 example items already specified in
      `docs/design/MAIN_BATTLE_SCENE_DESIGN.md` §4.3 are ready to transcribe directly.
...
- [ ] Audio pass per `docs/design/GAME_DESIGN_DOCUMENT.md` §15 (lowest-priority production item, cut first
      under time pressure per that doc's own ordering).
- [ ] Confirm `docs/building/BUILD_WINDOWS.md` / `docs/building/BUILD_WEB.md` still describe the actual build steps after all
...
a different shape and is tracked as **S1-S8** in `docs/progress_report/PROGRESS_REPORT.md` under Milestone status, with
its data contract in:

| Document | Covers |
|---|---|
| `docs/story/STORY_BIBLE.md` v3 | 70 floors = 10 acts x 7, the seal/cultivation ladder, 8 choices into 3 counters, 15 endings, rebirth rules |
| `docs/story/SIDE_STORIES.md` v3 | 10 flagship side stories (one per act, 5th floor), choice-gated and mirror variants, per-floor event pools |
| `docs/story/STORY_DATA_SCHEMA.md` v3 | per-floor files `F01..F70`, `stage.story`, 4 new condition types, maze scaling to 63x42, `endings.json`, `cycles.json`, save v3, 16 validators |
| `docs/design/ART_AND_ABILITY_DESIGN.md` | 50 enemies, 20 equipment, 36 statuses, ComfyUI prompts, 1/2/4-grid footprints, 1,006 frames + 469 statics, 3 MB budget |
...
new condition leaves and save `schemaVersion: 3`). See `docs/progress_report/PROGRESS_REPORT.md` for the per-phase evidence.
