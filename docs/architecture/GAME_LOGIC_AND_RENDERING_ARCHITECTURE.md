...
> `docs/design/FIGHT_SCENE_DESIGN.md` and `docs/design/MAIN_BATTLE_SCENE_DESIGN.md`).
>
...
|---|---|---|
| `docs/design/GAME_DESIGN_DOCUMENT.md` | Vision, pillars, story beats, bestiary, items, HUD intent | Source of truth for *why*; this doc is the *how*, system by system |
| `docs/design/FIGHTING_TALKING_DESIGN.md` | Baseline combat formula + dialogue JSON schema | Reused verbatim in §6 (damage) and §11 (dialogue) |
| `docs/design/FIGHT_SCENE_DESIGN.md` | Attack/Defense Power Bar minigame | Reused verbatim as the Battle System's round-resolution step (§6.3) |
| `docs/design/MAIN_BATTLE_SCENE_DESIGN.md` | Battle scene screen layout + Equipment System | Reused verbatim; this doc places it inside the full state machine (§4) |
| `docs/architecture/NODE_SYSTEM.md` | Scene-graph (`Node`/`GameObject`/`SpriteNode`/`TextNode`) + render binding | Reused as the **world-space** rendering layer (§3); this doc adds the **screen-space UI framework** on top (§2) |
...
above without hand-building a node tree per screen (as `docs/architecture/NODE_SYSTEM.md` currently does for the
...
Today, every interactive screen (`docs/architecture/NODE_SYSTEM.md`'s inventory, the store, the dialogue box) is
...
(per `docs/design/MAIN_BATTLE_SCENE_DESIGN.md`) two live timing bars with hit-zone bands — each of those is
...
stats-heavy game — cheap tooltips, so `docs/design/GAME_DESIGN_DOCUMENT.md` §13's "show the number, not just
...
(`docs/architecture/NODE_SYSTEM.md`) exactly as-is for everything world-space**: the 13×11 tile map, walking sprites,
...
3. **No embedded scripting, no XML skin files** — matches this project's explicit existing
   preference (`docs/design/MAIN_BATTLE_SCENE_DESIGN.md` §4.4: "a small enum rather than a generic scripting
...
game's existing 9-slice dialogue-panel art and HUD chrome (`docs/design/GAME_DESIGN_DOCUMENT.md` §14) is drawn
...
| 2 | Migrate **Inventory + Store** panels from hand-built `Node` trees to ImGui (these are the two screens `docs/architecture/NODE_SYSTEM.md`/`docs/design/FIGHTING_TALKING_DESIGN.md` already describe as grid/card layouts — most direct fit) | Low — behavior is already spec'd, only the construction method changes |
...
- **`Node` / `GameObject` / `SpriteNode` / `TextNode`** (`docs/architecture/NODE_SYSTEM.md`) — the scene-graph; world
...
preserves `docs/design/GAME_DESIGN_DOCUMENT.md` §13's "HUD is always visible, never fully obscured" rule even
...
`docs/design/GAME_DESIGN_DOCUMENT.md` §11). The separate `GAMEPLAY_ROGUELIKE_*` docs propose a much bigger
...
implicitly (`docs/design/GAME_DESIGN_DOCUMENT.md` §6: "tile clears" on win) but it was never named as a
...
**Note on `RespawnPending` and Pillar 3:** `docs/design/GAME_DESIGN_DOCUMENT.md` Pillar 3 ("losing costs time,
...
economy balance note in `docs/design/GAME_DESIGN_DOCUMENT.md` §9.
...
| `dialogue_gate` | Tile enter → `DIALOGUE` opens first (enemy's `enemy_*.json` flavor line, per `docs/design/FIGHTING_TALKING_DESIGN.md` §2.2) → player's choice resolves via the choice's `action` | "some may have story to show dialog then choice what to do and active a new story" |
...
`action.give`/door/flag verbs in `docs/design/FIGHTING_TALKING_DESIGN.md` §2.1) with three more explicit,
...
### 6.1 Base deterministic formula *(Established — `data/combat.json`, `docs/design/FIGHTING_TALKING_DESIGN.md` §1.2)*
...
effective.atk = player_base.atk (12)
              + level_up_bonus        (+2 per level, Established — docs/design/FIGHTING_TALKING_DESIGN.md §1.4)
              + gem/potion bonuses    (Established — data/items.json, data/store.json)
              + equipment flat bonus  (Proposed — docs/design/MAIN_BATTLE_SCENE_DESIGN.md §4, e.g. War Hammer +8 ATK)
...
Per `docs/design/FIGHT_SCENE_DESIGN.md` — each round is two player-timed Power Bar inputs, not an automatic
timer tick:
...
Equipment System's weapon/armor/talent slot design are specified in `docs/design/FIGHT_SCENE_DESIGN.md` and
`docs/design/MAIN_BATTLE_SCENE_DESIGN.md` — not repeated here.
...
`story_note` shown on floor entry (`docs/design/GAME_DESIGN_DOCUMENT.md` §5).
...
| `preview` | recommended-stats line, pulled straight from that floor's bestiary entries (`docs/design/GAME_DESIGN_DOCUMENT.md` §10) — this is what makes locked-but-visible floors still informative, not just grey boxes |
...
color per `docs/design/GAME_DESIGN_DOCUMENT.md` §14), each tile:
...
- **Input:** `npcId` → loads `data/dialogue/<npcId>.json` (Established schema:
  `start`/`nodes`/`text`/`choices`/`requires`/`action`, `docs/design/FIGHTING_TALKING_DESIGN.md` §2.1).
...
- **Renders via:** ImGui dialogue box (§2.3 Phase 3), 9-slice background drawn by the scene-graph
  underneath it (unchanged art direction, `docs/design/GAME_DESIGN_DOCUMENT.md` §13/§14).
...
|---|---|---|
| Inventory | `data/items.json`, 9-grid per `docs/architecture/NODE_SYSTEM.md` | Grid construction moves from hand-built `Node` tree to ImGui `ImageButton` grid + drag-drop (§2.3 Phase 2); item effect text still sourced from the same `desc`/`effect_text` fields |
| Equipment | `data/equipment.json` (Proposed schema, `docs/design/MAIN_BATTLE_SCENE_DESIGN.md` §4.4) | Three slots (weapon/armor/talent), each swap re-derives §6.2's effective stats + Power Bar parameters |
...
## 15. Open Design Checklist *(extends `docs/design/GAME_DESIGN_DOCUMENT.md` §17)*
...
Project sources (verified directly against files, same convention as `docs/design/GAME_DESIGN_DOCUMENT.md`):
`TOMS/src/engine/render_iface.h`, `TOMS/docs/architecture/NODE_SYSTEM.md`, `TOMS/docs/design/FIGHT_SCENE_DESIGN.md`,
`TOMS/docs/design/MAIN_BATTLE_SCENE_DESIGN.md`, `TOMS/docs/design/GAME_DESIGN_DOCUMENT.md`,
