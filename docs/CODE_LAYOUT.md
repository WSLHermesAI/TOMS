...
| `floor_table.h` / `floor_table.cpp` | 78 / 121 | **S3.5**：`toms::FloorTable` — 70 層塔的**有序資料**：`seq` 由 `nextFloor` 鏈推導（不信檔名），含 act／indexInAct／role／seal／名稱鍵，以及 `mapRelPath()`（一般層→生成圖，頭目層→手寫 `stageNN.json`）。`loadStage()` 用它把 `st.id`／`st.index`／`st.name` 與 **`st.up`／`st.down`** 換成樓層值，樓梯程式因此不必改就能逐層推進。同樣不認得 Renderer／Stage／Game，可獨立測試。 |
...
| `*_test.cpp` | — | 純邏輯測試（不開視窗）：`camera_test`、`run_state_test`（S3，45 項）、`floor_table_test`（S3.5，990 項）、`title_screen_test`、`equipment_test`、`mission_test`、`save_test`、`condition_eval_test`、`entity_status_test`、`story_controller_test`、`encounter_resolve_test`。 |
